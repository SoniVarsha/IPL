$(document).ready(function(){
    ...
    if(localStorage.getItem("search")==null){
        ...
        let exp = JSON.parse(localStorage.getItem("search"));
        let flag =0;
    ...
    let n1 = $("<p>").html(`player name: <span>${data.name}</span>`).attr({
        class : "player-info"
    })
    let n2 = $("<p>").html(`description: <span>${data.description}</span>`).attr({
        class : "player-info"
    })
    let n3 = $("<p>").html(`team: <span>${data.team}</span>`).attr({
        class : "player-info"
    })
    let n4 = $("<p>").html(`role: <span>${data.role}</span>`).attr({
        class : "player-info"
    })
    let n5 = $("<p>").html(`playing status: <span>${data.playingStaus}</span>`).attr({
        class : "player-info"
    })
    let n6 = $("<p>").html(`price: <span>${data.price}</span>`).attr({
        class : "player-info"
    })
    $(card).append(img1);
    ...
    $("#sub-1").click(function(e) {
        ...
        for( let i = 0; i<=data.length; i++ ){
          if(data[i].key == n4){   //check the team name
             let obj = {
